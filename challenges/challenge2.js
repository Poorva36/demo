...
0. Create a function 'checkDogs', which accepts 2 arrays
...
// phela include, last exclude---
let checkDogs = function(dogsJulia , dogsKate){
    const dogsJuliasCorrected = dogsJulia.slice(2,4)
    // console.log(dogsJuliasCorrected);
    const totalDogs = dogsJuliasCorrected.concat(dogsKate)
    console.log(totalDogs);

    totalDogs.forEach( function(dogu, index){

        if(dogu < 3){
            console.log(`Dog number ${index+1} is a puppy, and is ${dogu}
            years old`);
        }
        else if(dogu >= 3){
            console.log(`Dog number ${index+1} is an adult, and is ${dogu}
            years old`);
        }
    })
}
